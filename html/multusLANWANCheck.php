...
$PageIdentifier = "multusLANWANCheck";
...
if ( ! isset($_SESSION[$SessionClassIdentifier]))
{
	...
	$ObjmultusLANWANCheckClass = new ClassmultusLANWANCheck($PageConfig, $ObjUsers->CurrentEditRightLevel, "Setup ".$PageDescription, $PageIdentifier, $RunOnce, $Instance);
}
else
{
	$ObjmultusLANWANCheckClass = unserialize(base64_decode($_SESSION[$SessionClassIdentifier]));
}

// check on prior action
$ObjmultusLANWANCheckClass->HandleApplyButton();

if ($ObjmultusLANWANCheckClass->bIniNeedsToBeWritten)
{
	// Save changes into session
	$_SESSION[$SessionClassIdentifier] = base64_encode(serialize($ObjmultusLANWANCheckClass)); 

	$ObjmultusLANWANCheckClass->writeIni($ObjmultusLANWANCheckClass->ini, $PageConfig);
}

$ObjmultusLANWANCheckClass->HandleChangesToRestart();


/////// Now here comes the html stuff and the selction
$ObjmultusLANWANCheckClass->PutTheTable();

/////// Put the RebootButton
$ObjmultusLANWANCheckClass->PutRebootButtons();
